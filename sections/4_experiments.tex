...
the capability-relevant protocol. \emph{Static} matches \emph{Oracle} through the warmup
phase but collapses each time the distribution shifts. \emph{Online adapt} closes most of
the gap within a small detection latency (typically 2--7 cases per shift) and tracks
\emph{Oracle} for the remainder of each phase. Aggregated over the full stream, the
ordering holds across all five backbones and three stream constructions: on GPT-5.4
capability-relevant, \emph{Online adapt} reaches $0.870$ versus $0.636$ static and $0.899$
oracle; on stress, \emph{Direct} collapses to $0.026$, \emph{Static} reaches $0.391$, and
\emph{Online adapt} recovers to $0.947$; on the natural mix the gain shrinks but remains
positive ($0.830$ vs.\ $0.800$ static).

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/distribution_shift_timeseries.pdf}
  \caption{%
    \textbf{Online adaptation under distribution shift (GPT-5.4, capability-relevant).}
    Rolling accuracy along the stream for each policy. Shaded bands mark the four stream
    phases. \emph{Static} matches the oracle in the warmup phase but collapses at each
    shift; \emph{Online adapt} recovers within a short detection latency and tracks the
    oracle for the remainder of each phase.
  }
