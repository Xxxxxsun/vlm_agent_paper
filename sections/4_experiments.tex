...
finally a four-way mixed phase including V$^*$. The replay reuses completed per-case
outputs, isolating the adaptation policy from the cost of regenerating capabilities
online. We compare \emph{Direct} (no capabilities), \emph{Static} (capabilities evolved
...
\begin{figure}[t]
  ...
  \caption{%
    ...
    Rolling accuracy for each policy along a 208-step stream that drifts from HRBench
    through MathVista, ChartQA, and a four-way mix. \emph{Static} lags at each shift;
    \emph{Online adapt} catches up to \emph{Oracle} within a small detection latency.
  }
