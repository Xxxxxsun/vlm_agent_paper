...
Figure~\ref{fig:distribution_shift} plots rolling accuracy over the stream for GPT-5.4 on
the natural protocol. All four policies share the same trajectory through the warmup
phase; at each subsequent shift, \emph{Static}'s accuracy lags because its
HRBench-evolved capabilities no longer match the incoming distribution, while
\emph{Online adapt} re-evolves on the new sub-stream and catches up to \emph{Oracle}
within a small detection latency (2--7 cases per shift). Aggregated over the full stream,
the ordering holds across all five backbones and three stream constructions: on GPT-5.4
natural, \emph{Online adapt} reaches $0.832$ versus $0.736$ static and $0.851$ oracle;
on the capability-relevant protocol, which selects cases whose outcome depends on the
matching capability, the gap widens to $0.870$ vs.\ $0.636$ static and $0.899$ oracle;
on the stress protocol, \emph{Direct} collapses to $0.026$, \emph{Static} reaches $0.391$,
and \emph{Online adapt} recovers to $0.947$.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/distribution_shift_timeseries.pdf}
  \caption{%
    \textbf{Online adaptation under distribution shift (GPT-5.4, natural stream).}
    Rolling accuracy for each policy along the 208-step stream. Phase labels above the
    plot mark the newly introduced family at each shift; earlier families remain present
    in the mix. \emph{Static} lags at each shift; \emph{Online adapt} catches up to
    \emph{Oracle} within a small detection latency.
  }
