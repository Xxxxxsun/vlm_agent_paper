...
\multirow{4}{*}{GPT-4o}
  ...
  & Full               &$0.6886 {\pm} 0.0052$  &$0.7011 {\pm} 0.0067$  &$0.6313 {\pm} 0.0101$  &$0.7799 {\pm} 0.0045$  \\
...
\caption{%
  ...
  Simulates a real online user-traffic distribution that drifts over time. The stream
  begins on HRBench and later introduces MathVista, ChartQA, and V$^*$ cases.
  \emph{Static} keeps the initial-distribution capabilities unchanged; \emph{Online adapt}
  detects the drift and updates skills and tools on the new sub-stream; \emph{Oracle} has
  capabilities pre-evolved on every family. Rows are grouped by backbone; the three
  protocols within each block differ only in how the replay stream is constructed.
  Standard deviations are temporary layout placeholders and must be replaced with
  seed-based estimates before submission.
}
