...
\caption{%
  \textbf{Exp~I: Online adaptation to distribution shift.}
  \emph{Static}: capabilities evolved on the initial distribution only;
  \emph{Online adapt}: \sysname{} updates skills and tools after detecting the shift;
  \emph{Oracle}: capabilities pre-evolved on every family (upper bound).
  Rows are grouped by backbone; the three protocols within each block vary only in how
  the replay stream is constructed (see prose). Standard deviations are temporary
  placeholders.
}
