...
such shifts and re-equip its capabilities online, without weight updates and without the
practitioner having to anticipate the shift or prepare a per-family training set.

\paragraph{Streaming evolution.}
On a query stream, \sysname{} continuously samples a sub-training set from the most recent
queries and runs the evolution loop of Section~\ref{sec:method_loop} on them. The resulting
library $\mathcal{C}$ is naturally partitioned into \emph{task-family packs}
$\{\mathcal{C}_f\}_{f}$, one per family the stream has visited.

\paragraph{Shift detection and re-equipping.}
\sysname{} monitors a rolling window over the latest queries and identifies the dominant
task family from their question and image features. When the detected family differs from
the currently active pack, the system re-equips after a small confirmation latency: an
existing matching $\mathcal{C}_f$ is activated, and if the family is new, the evolution
loop is launched on the new sub-stream to grow one. We treat this autonomous online
adaptation as one of the method's contributions; its swap policy is evaluated in
Experiment~I (distribution-shift adaptation, Section~\ref{sec:exp1}) against a static-pack
baseline and an oracle that has all packs available from the start.
