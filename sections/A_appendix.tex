...
lateral-position tool and paired mastery skill, improving the selection score from
0.5333 to 0.6000. The accepted capability targets the same narrow HRBench-cross
bottleneck where several full-grid runs generated similar position-marking tools, so we
report this row as a positive existence check for the guarded single-candidate path
rather than as a direct comparison of $M{=}1$ versus multi-candidate exploration.
...
Read together, the mechanism ablations triangulate the same claim from several angles.
The completed HRBench probe in Table~\ref{tab:ablation_mechanism} rules out a pure
prompt-engineering explanation and points to the visual-tool/artifact path as the active
mechanism on perceptual tasks. The guarded 30-case pilot in
Table~\ref{tab:ablation_guarded_pilot} further confirms that (i) the evolution loop can
promote real generated tools when its acceptance criterion is enforced, (ii) text-only
diagnosis fails to materialise a usable visual artifact, and (iii) removing the paired
mastery skill turns an executable tool into a harmful subset-level intervention.
Failure-Only Diagnosis and Single-Candidate also promoted tools in the pilot on the
narrow HRBench-cross cluster they tested.
The ChartQA polarity stress test in Table~\ref{tab:ablation_diagnosis_polarity}
adds a complementary signal: correct-only diagnosis cannot enter the generation path,
while error-only diagnosis repeatedly regenerates broad tools without promoting any
capability. This supports the full loop's use of contrast between solved and unsolved
cases when forming reusable skills and tools.
